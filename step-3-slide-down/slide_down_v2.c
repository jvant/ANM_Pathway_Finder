...
#include "slide_down_v2.h"



/* Check if the step-size if adequate. Return 1 if works, 0 if does not work*/
int Check_step_size(int Num_particles, double force_constant, double cutoff, double **pair_distances_structure, double *Reference_x, double *Reference_y, double *Reference_z, double rmsd_pathway, double delta_rmsd_zero, FR_DAT *fr_start, double step_size)
{
  FR_DAT fr;
  int i;
  double rmsd;
  ...
  /* Calculate force*/
  getforces_enm(&fr, pair_distances_structure, cutoff, force_constant);
  /* Move by one step*/
  for(i = 0 ; i < Num_particles ; i++)
    {
