...
void Read_input(int *Num_particles, int *struct_index, double *cutoff, double *force_constant, double *step_size, double *rmsd_pathway, double *delta_rmsd_zero, double *energy_from_ref_tol, long int *Max_num_iterations)
{
  FILE *in;
  char key[1000];
...
int main()
{
  ...
  Read_input(&Num_particles, &struct_index, &cutoff, &force_constant, &step_size, &rmsd_pathway, &delta_rmsd_zero, &energy_from_ref_tol, &Max_num_iterations);
  ...
  while(is_step_size_adequate == 0)
    {
       is_step_size_adequate = Check_step_size(Num_particles, force_constant, cutoff, pair_distances_structure, Reference_x, Reference_y, Reference_z, rmsd_pathway, delta_rmsd_zero, &fr_start, step_size);
  ...
  getforces_enm(&fr_ref, pair_distances_structure, cutoff, force_constant);
  energy_ref = fr_ref.U;

  Num_struct_written = 0;
  Slide_down_single_step_size(Num_particles, force_constant, cutoff, pair_distances_structure, Max_num_iterations, Reference_x, Reference_y, Reference_z, struct_index, rmsd_pathway, delta_rmsd_zero, &fr_ref, &fr_start, step_size, energy_ref, energy_from_ref_tol, &Num_struct_written, &fr_end, &iteration_number_end, step_size_reduction_factor);
  ...
  getforces_enm(&fr_end, pair_distances_structure, cutoff, force_constant);
